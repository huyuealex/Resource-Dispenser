	...
	for (i = MAXN_PRIO; i >= 0; i--) {
		if (!list_empty(&nd->prio_queue[i])) {
			if (last_prio <= i || force || time_after(jiffies, dontuntil)) {
				...
				cnt_processes[i]++;
				dontuntil = jiffies + msecs_to_jiffies(TIME_AFTER_DISPATCH); // 20ms后才能dispatch普通进程的请求
				mod_timer(&nd->resume_timer, jiffies + msecs_to_jiffies(TIME_RESET)); // 定时器在30ms后尝试重启io
				last_prio = i;
				return 1;
			}
		}
...
static void rd_add_request(struct request_queue *q, struct request *rq)
{
	...
	for (i = 0; i < LEN; i++) {
		if (hash_process[key][i].alive == 1 && current->pid == hash_process[key][i].pid) {
			list_add_tail(&rq->queuelist, &nd->prio_queue[hash_process[key][i].prio]);
			find = 1;
			break;
		}
	}
	if (!find) {
		list_add_tail(&rq->queuelist, &nd->prio_queue[0]);
	}
}
		...
		unsigned long len, void *data )
{
	...
	if (copy_from_user(proc_writebuf, buff, len))
		return -EFAULT;
	proc_writebuf[len] = 0; // 无论用户传入串是否以0结尾,保证proc_writebuf以0结尾
...
static int rdnice_read( char *page, char **start, off_t off,int count, int *eof, void *data )
{
	...
	if (len == 0)
		len = sprintf(page, "No data in hash\n");
	return len;
}
